...
class EuclideanCodebook(nn.Module):
    ...
    def forward(self, x):
        ...
        # dist shape = [batch * data_amount, codebook_size]
        dist = -(
        ...
        # embed_ind shape = [batch * data_amount]
        embed_ind = gumbel_sample(dist, dim = -1, temperature = self.sample_codebook_temp)
        # embed_onehot shape = [batch * data_amount, codebook_size]
        ...
        # MOD: calcualte perplexity
        avg_probs = torch.mean(embed_onehot, dim=0)
        self.entropy = -torch.sum(avg_probs * torch.log(avg_probs + 1e-10))
        self.perplexity = torch.exp(self.entropy)
        self.perplexity_rate = self.perplexity / self.codebook_size
...
class VectorQuantize(nn.Module):
    ...
    def forward(self, x):
        ...
        return quantize, embed_ind, loss, self._codebook.perplexity_rate # MOD
